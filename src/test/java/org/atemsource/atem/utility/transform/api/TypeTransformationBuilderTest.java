...
public class TypeTransformationBuilderTest {
	...
	private EntityTypeTransformation<?, ?> createEntityBTransformation() {
		if (entityBTransformation == null) {
			...
			bBuilder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
					.createBuilder("testSingleAssociationB"));
	...
	public void testAssociativeCollection() {
		TypeTransformationBuilder<?, ?> builder = transformationBuilderFactory
				.create();
		builder.setSourceType(EntityA.class);
		builder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
				.createBuilder("testAssociativeCollection"));
		builder.transformCollection().from("list").to("list")
				.convert(createEntityBTransformation());
		EntityTypeTransformation<EntityA, DynamicEntity> entityTypeTransformation = (EntityTypeTransformation<EntityA, DynamicEntity>) builder
		...
		bs.add(b2);
		a.setList(bs);
		DynamicEntity b = entityTypeTransformation.getAB().convert(a,
	...
	public void testAssociativeMap() {
		...
		builder.setSourceType(EntityA.class);
		builder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
				.createBuilder("testAssociativeMap"));
		builder.transformMap().from("map").to("map")
				.convert(createEntityBTransformation());
		EntityTypeTransformation<EntityA, DynamicEntity> entityTypeTransformation = (EntityTypeTransformation<EntityA, DynamicEntity>) builder
				.buildTypeTransformation();
		EntityType<?> typeB = entityTypeTransformation.getEntityTypeB();
		...
		DynamicEntity b = entityTypeTransformation.getAB().convert(a,
				new SimpleTransformationContext());
		Map<String, DynamicEntity> actualMap = (Map<String, DynamicEntity>) b
				.get("map");
	...
	public void testPrimitiveCollection() {
		...
		builder.setSourceType(EntityA.class);
		builder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
				.createBuilder("testPrimitiveCollection"));
	...
	public void testPrimitiveMap() {
		...
		builder.setSourceType(EntityA.class);
		builder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
				.createBuilder("testPrimitiveMap"));
	...
	public void testSingleAssociation() {
		...
		builder.setSourceType(EntityA.class);
		builder.setTargetTypeBuilder(dynamicEntityTypeSubrepository
				.createBuilder("testSingleAssociation"));
		...
		EntityA a = new EntityA();
		a.setEntityB(b);
		DynamicEntity a2 = entityTypeTransformation.getAB().convert(a,
				new SimpleTransformationContext());
	...
	public void testSinglePrimitive() {
		...
		EntityTypeTransformation<EntityB, DynamicEntity> entityTypeTransformation = (EntityTypeTransformation<EntityB, DynamicEntity>) builder
				.buildTypeTransformation();
		EntityType<?> typeB = entityTypeTransformation.getEntityTypeB();
	...
	public void testGeneric() {
		...
		transformCustom.getTargetTypeBuilder().addSingleAttribute(
				"derivedValue", entityTypeRepository.getType(String.class));
		transformCustom.from("booleanO").from("intO")
				.transform(new JavaTransformation<EntityA, DynamicEntity>() {

					@Override
					public void mergeAB(EntityA a, DynamicEntity b) {
						b.put("derivedValue",
								a.getBooleanO() + "" + a.getIntO());
					}
